...
export function UptadeProfilePhoto() {
  ...
  const { mutate: handleChangeProfilePhoto, isPending } = useMutation({
    ...
    onSuccess: () => {
      setOpenModal(false);
      toast.success("photo updated successfully", { theme: "dark" });
      getProfileData(localStorage.getItem("token"));
    },
    onError: () => {
      toast.error("photo updated failed", { theme: "dark" });
    },
  });
  return (
    <>
      <FaEdit
        className="cursor-pointer  p-2 size-9 rounded absolute bottom-0 right-0"
      ...
      >
        ...
        <ModalBody>
          ...
          >
            ...
            <ButtonApp>{isPending ? "Uploading.." : "Upload"}</ButtonApp>
          </form>
        </ModalBody>
      </Modal>
    </>
